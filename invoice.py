'''
INVOICE_GENERATOR USING PYTHON
'''
...
class Terminal_user_interface():
    def pretty_header(self, name: str, invoice_num, ):
        ...
        return f"-" * 67 + f"\n{name}{space * space_num}Invoice Number: {invoice_num}"

    def pretty_print_line(self, item, quantity, rate, amount):
        space = " "
        space1 = 43 - len(item) - len(str(quantity))
        space2 = 9 - len(str(rate))
        space3 = 12 - len(str(amount))
        return f"{item}{space * space1}{quantity}{space * space2}{rate} Rs.{space * space3}{amount} Rs."

    def pretty_total(self, t):
        space = " "
        spaces = 59 - len(str(t))
        return f"{space * spaces}TOTAL: {t} Rs."
